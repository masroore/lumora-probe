...
"""Bundled seed analyzers implemented against the public plugin SDK."""
...
from .contracts import AnalysisContextDTO, FindingDTO

BUNDLED_RULE_SET_VERSION = "bundled-v1"


class BundledAnalyzer(Protocol):
    """Public analyzer shape used by the bundled plugin."""

    rule_id: str
    rule_version: str

    def evaluate(self, context: AnalysisContextDTO) -> Iterable[FindingDTO]: ...
...
class BundledRulePlugin:
    """First-party seed rule plugin using only public SDK DTOs."""

    @hookimpl
    def analyze(self, context: AnalysisContextDTO) -> Iterable[FindingDTO]:
        for rule in bundled_rules():
            yield from rule.evaluate(context)


bundled_plugin = BundledRulePlugin()

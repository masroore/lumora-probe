...
"""DCMTK interoperability scenarios for Lumora's DICOM relay."""
...
@pytest.mark.asyncio
async def test_dcmtk_echoscu_verifies_lumora_relay(free_port: int) -> None:
...
@pytest.mark.asyncio
async def test_dcmtk_storescu_sends_synthetic_instance_through_relay(
    free_port: int,
) -> None:
    relay = _relay(free_port)
    await relay.start()
    ...
    assert "instance-01.dcm" in result.stderr


@pytest.mark.asyncio
async def test_dcmtk_calling_ae_rejection_is_explicit_and_relay_recovers(
    free_port: int,
) -> None:
    ...
    assert "Calling AE Title Not Recognized" in denied.stderr
